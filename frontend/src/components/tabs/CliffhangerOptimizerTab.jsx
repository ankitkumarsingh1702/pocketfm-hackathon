...
import { MetricNumber, ScoreGauge, SurfaceCard } from '../primitives'
import { EmptyState, ErrorState, LoadingState } from '../StateViews'

function EndingCard({ label, text, score, gaugeLabel, elevated }) {
  return (
    <SurfaceCard elevated={elevated}>
      <div className="label-upper" style={{ marginBottom: 20, fontSize: 11 }}>
        {label}
      </div>
 ...
 */
export default function CliffhangerOptimizerTab({ loading, error, data }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 40, paddingTop: 8 }}>
      ...
      >
        <EndingCard label="Original Ending" text={data.original} score={data.before} gaugeLabel="ORIGINAL" />
        <EndingCard
