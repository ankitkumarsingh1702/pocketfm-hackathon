"""
Mood-First Search — API router.

Exposes the frozen contract as an ``APIRouter`` that the Simulated Studio app
mounts under ``/api/mood`` (see ``app/main.py``). The whole retrieval stack —
parser, store, retriever, reranker, entry-point resolver — sits behind these
handlers; the frontend only ever sees the ``SearchResponse`` shapes.

The engine, episode store and demo panel are built once at import time and fall
back to in-memory fixtures when ``MOOD_INDEX`` / ``MOOD_EPISODES`` / ``DEMO_PANEL``
are unset, so the router always boots with something to serve.
"""
...
from app.mood.episodes import EpisodeStore, derive_stub_episodes
from app.mood.baseline import GenreBaseline
from app.mood.parser import default_parser
from app.mood.persona import build_demo_panel, load_profiles
from app.mood.mood_config import (
    MOOD_STARTERS, SLIDER_LABELS, SUPPORT_RESOURCES, build_clarifying_question,
)
from app.mood.rerank import default_reranker
from app.mood.schemas import ClarifyAnswerRequest, MoodQuery, RefineRequest, SearchResponse
...
if Path(PANEL_PATH).exists():
    ...
    from app.mood.evaluate import synthetic_personas
...
@router.get("/health")
def health():
    from app.mood.embeddings import describe

    return {
        "arcs": len(engine.store),
        "series": len({f.series_id for f in engine.store.fingerprints}),
        # NOTE: `audio_verified` is a catalog flag, not evidence of an audio
        # pipeline -- there isn't one in this build. Named `flagged_audio` so a
        # nonzero count here can't be read as "N items went through ASR".
        "flagged_audio": sum(f.audio_verified for f in engine.store.fingerprints),
        "episodes": len(EPISODES),
        "reranker": type(engine.reranker).__name__,
        "refine_reranker": type(engine.refine_reranker).__name__,
        "parser": "llm" if parser.client else "heuristic",
        **describe(engine.store.embedder),
    }
...
def clarify(req: ClarifyAnswerRequest):
    ...
    if req.option_id:
        from app.mood.mood_config import DEFAULT_CLARIFY, SESSION_CLARIFY
...
def refine(req: RefineRequest):
    t0 = time.time()
    # Keep the profile, don't just take the query: the finished-series hard mask
    # lives on the profile, and dropping it here let a slider drag resurface a
    # series the listener had already finished.
    query, profile = _SESSIONS.get(req.query_id) or (parser.parse(""), None)
    destination = req.shelf_id.replace("shelf_", "")
    shelf = engine.refine(
        query, destination, req.current_axes, req.slider_deltas, profile=profile
    )
    if shelf is None:
